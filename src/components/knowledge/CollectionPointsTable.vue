<template>
  <div class="points-table">
    ...
    <DataTable 
      ...
      stripedRows
    >
      <Column field="payload.source_term" header="Исходный термин" style="width: 220px;" sortable>
        <template #body="slotProps">
          <span class="cell-truncate">{{ slotProps.data.payload.source_term || '-' }}</span>
        </template>
      </Column>
      
      <Column field="payload.target_term" header="Переведенный термин" style="width: 220px;" sortable>
        <template #body="slotProps">
          <span class="cell-truncate">{{ slotProps.data.payload.target_term || '-' }}</span>
        </template>
      </Column>

      <Column field="payload.context" header="Категория" style="width: 200px;">
        <template #body="slotProps">
          <span class="truncate">{{ slotProps.data.payload.context || '-' }}</span>
        </template>
      </Column>

      <Column field="payload.tags" header="Теги" style="width: 160px;">
        <template #body="slotProps">
          <div v-if="slotProps.data.payload.tags && slotProps.data.payload.tags.length > 0" class="tag-chips">
            <Chip 
      ...
      <Column v-if="isClientCollection" field="payload.client" header="Клиент" style="width: 200px;">
        <template #body="slotProps">
          <span class="truncate">{{ slotProps.data.payload.client || '-' }}</span>
        </template>
      </Column>
