<template>
  <div class="history-table">
    ...
    >
      ...
      <Column field="client_name" header="Клиент" style="min-width: 160px;" sortable>
        <template #body="slotProps">
          <Tag :value="slotProps.data.client_name" severity="info" class="client-tag" />
        </template>
      </Column>
      
      <Column field="source_lang" header="SRC" style="width: 60px;" sortable>
        <template #body="slotProps">
          <Tag :value="slotProps.data.source_lang?.toUpperCase()" severity="success" />
        </template>
      </Column>
      
      <Column field="target_lang" header="TRG" style="width: 60px;" sortable>
        <template #body="slotProps">
          <Tag :value="slotProps.data.target_lang?.toUpperCase()" severity="warning" />
        </template>
      </Column>
      
      <Column field="source_text" header="Исходный текст" style="min-width: 180px;">
        <template #body="slotProps">
          <div class="text-truncate">{{ truncateText(slotProps.data.source_text, 50) }}</div>
        </template>
      </Column>
      
      <Column field="translated_text" header="Перевод" style="min-width: 180px;">
        <template #body="slotProps">
          <div class="text-truncate">{{ truncateText(slotProps.data.translated_text, 50) }}</div>
        </template>
      </Column>
...
const filteredHistoryItems = computed(() => {
  ...
  return props.historyItems.filter(item => item.client_id === clientIdFilter.value);
});
